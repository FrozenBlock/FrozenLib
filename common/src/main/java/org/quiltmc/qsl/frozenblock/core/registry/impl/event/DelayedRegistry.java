...
public abstract class DelayedRegistry<T> implements WritableRegistry<T> {
	private static Function<WritableRegistry<?>, DelayedRegistry<?>> factory = null;

	@SuppressWarnings("unchecked")
	public static <T> DelayedRegistry<T> create(WritableRegistry<T> registry) {
		if (factory == null) throw new IllegalStateException("DelayedRegistry factory not initialized — platform init missing");
		return (DelayedRegistry<T>) factory.apply(registry);
	}

	public static void setFactory(Function<WritableRegistry<?>, DelayedRegistry<?>> newFactory) {
		factory = newFactory;
	}
