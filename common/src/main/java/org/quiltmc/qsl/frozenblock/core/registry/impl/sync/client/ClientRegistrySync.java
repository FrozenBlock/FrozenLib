...
public final class ClientRegistrySync {
	...
	private static void handleModProtocol(ServerPackets.ModProtocol modProtocol, ConfigPacketSender sender) {
	...
	private static void handleEndPacket(ServerPackets.End end, ConfigPacketSender sender) {
		syncVersion = -1;
	...
	private static void sendSupportedModProtocol(ConfigPacketSender sender, Object2IntOpenHashMap<String> values) {
		sender.sendPacket(new ClientPackets.ModProtocol(values));
	}

	private static void handleErrorStylePacket(ServerPackets.ErrorStyle errorStyle) {
		errorStyleHeader = errorStyle.errorHeader();
		errorStyleFooter = errorStyle.errorFooter();
		showErrorDetails = errorStyle.showError();
	}

	private static void handleHelloPacket(ServerPackets.Handshake handshake, ConfigPacketSender sender) {
