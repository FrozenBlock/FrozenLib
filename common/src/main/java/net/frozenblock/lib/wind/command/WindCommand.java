...
public class WindCommand {
	...
	public static LiteralArgumentBuilder<CommandSourceStack> buildSubCommand() {
		...
		overrideBuilder
			...
			.then(Commands.literal("disable").executes(context -> disableWindOverride(context.getSource())));
	...
	private static int disableWindOverride(CommandSourceStack source) {
		final ServerLevel level = source.getLevel();
		final WindManager windManager = WindManager.getOrCreate(level);
		if (windManager.windOverride.isEmpty()) {
			source.sendFailure(Component.translatable("commands.wind.override.disable.failure"));
			return 0;
		}

		windManager.windOverride = Optional.empty();
		windManager.trySync(level);
		source.sendSuccess(() -> Component.translatable("commands.wind.override.disable.success"), true);
		return 1;
	}
	...
	private static int setAndEnableWindOverride(CommandSourceStack source, double x, double y, double z) {
		...
		windManager.trySync(level);
		source.sendSuccess(() -> Component.translatable("commands.wind.override.success", x, y, z), true);
		return 1;
	}
