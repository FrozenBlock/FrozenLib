...
public final class TickEvents {
	/**
	 * Called at the start of the {@link MinecraftServer} tick.
	 * <p>
	...
	public static final Event<StartTick> START_SERVER_TICK = EventRegistry.createEnvironmentEvent(StartTick.class, callbacks -> server -> {
		for (StartTick event : callbacks) event.onStartTick(server);
	});

	/**
	 * Called at the end of the {@link MinecraftServer}'s tick.
	 * <p>
	 * When the dedicated server is "paused", this event is not invoked.
	 */
	public static final Event<EndTick> END_SERVER_TICK = EventRegistry.createEnvironmentEvent(EndTick.class, callbacks -> server -> {
		for (EndTick event : callbacks) event.onEndTick(server);
	});

	/**
	 * Called at the start of a {@link ServerLevel}'s tick.
	 *
	...
	/**
	 * Called at the end of a {@link ServerLevel}'s tick.
	 * <p>
