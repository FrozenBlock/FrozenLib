...
public final class ServerLivingEntityEvents {
	 ...
	 */
	public static final Event<AllowDamage> ALLOW_DAMAGE = EventRegistry.createEnvironmentEvent(AllowDamage.class, callbacks -> (entity, source, amount) -> {
	 ...
	 */
	public static final Event<AfterDamage> AFTER_DAMAGE = EventRegistry.createEnvironmentEvent(AfterDamage.class, callbacks -> (entity, source, baseDamageTaken, damageTaken, blocked) -> {
		for (AfterDamage callback : callbacks) callback.afterDamage(entity, source, baseDamageTaken, damageTaken, blocked);
	});
	 ...
	 * </ul>
	 */
	public static final Event<AllowDeath> ALLOW_DEATH = EventRegistry.createEnvironmentEvent(AllowDeath.class, callbacks -> (entity, damageSource, damageAmount) -> {
		for (AllowDeath callback : callbacks) {
			if (!callback.allowDeath(entity, damageSource, damageAmount)) return false;
		}
	 ...
	 */
	public static final Event<AfterDeath> AFTER_DEATH = EventRegistry.createEnvironmentEvent(AfterDeath.class, callbacks -> (entity, damageSource) -> {
		for (AfterDeath callback : callbacks) callback.afterDeath(entity, damageSource);
	});
	 ...
	 */
	public static final Event<MobConversion> MOB_CONVERSION = EventRegistry.createEnvironmentEvent(MobConversion.class, callbacks -> (previous, converted, keepEquipment) -> {
		for (MobConversion callback : callbacks) callback.onConversion(previous, converted, keepEquipment);
	});
