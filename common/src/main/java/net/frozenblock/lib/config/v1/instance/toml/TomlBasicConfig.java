...
package net.frozenblock.lib.config.v1.instance.toml;
...
public class TomlBasicConfig<T> extends BasicConfig<T> {
	public static final String EXTENSION = "toml";

	private final TomlWriter tomlWriter;

	public TomlBasicConfig(String modId, Class<T> config) {
		this(modId, config, new TomlWriter.Builder());
	}

	public TomlBasicConfig(String modId, Class<T> config, TomlWriter.Builder builder) {
		this(modId, config, makePath(modId, EXTENSION), builder);
	}

	public TomlBasicConfig(String modId, Class<T> config, Path path, TomlWriter.Builder builder) {
