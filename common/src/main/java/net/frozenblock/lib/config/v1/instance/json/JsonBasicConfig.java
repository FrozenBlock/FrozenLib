...
package net.frozenblock.lib.config.v1.instance.json;
 ...
 */
public class JsonBasicConfig<T> extends BasicConfig<T> {
	private final Jankson jankson;
	private final JsonType type;

	public JsonBasicConfig(String modId, Class<T> config) {
		this(modId, config, JsonType.JSON);
	}

	public JsonBasicConfig(String modId, Class<T> config, JsonType type) {
		this(modId, config, makePath(modId, type.getSerializedName()), type);
	}

	public JsonBasicConfig(String modId, Class<T> config, Path path, JsonType type) {
		super(modId, config, path);

		final var janksonBuilder = Jankson.builder();
		this.jankson = BasicConfigSerialization.createJankson(janksonBuilder, modId);
		this.type = type;
