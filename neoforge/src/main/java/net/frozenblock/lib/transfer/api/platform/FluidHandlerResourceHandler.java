...
public final class FluidHandlerResourceHandler implements ResourceHandler<FluidResource> {
	...
	public int insert(int index, FluidResource resource, int amount, TransactionContext transaction) {
		...
		final FluidTransferVariant variant = TransferApiImpl.fromNeoForge(resource);
		final FluidTransferVariant current = this.handler.getVariant(index);
		if (!current.isBlank() && !current.equals(variant)) return 0;
		if (!this.handler.isValid(index, variant)) return 0;

		final int currentAmount = this.handler.getAmount(index);
		final int inserted = Math.min(amount, this.handler.getCapacity(index, variant) - currentAmount);
	...
	public int extract(int index, FluidResource resource, int amount, TransactionContext transaction) {
		...
		final int currentAmount = this.handler.getAmount(index);
		final int extracted = Math.min(amount, currentAmount);
		if (extracted <= 0) return 0;

		this.journals[index].updateSnapshots(transaction);
		final int remaining = currentAmount - extracted;
