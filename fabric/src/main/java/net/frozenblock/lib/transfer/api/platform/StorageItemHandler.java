...
public final class StorageItemHandler implements ItemHandler {
	...
	public ItemStack getStack(int index) {
		if (this.slotted != null) {
			final SingleSlotStorage<ItemVariant> slot = slot(index);
			return slot.getResource().toStack((int) slot.getAmount());
		}

		final StorageView<ItemVariant> view = firstView();
	...
	public int getCapacity(int index, ItemStack stack) {
		...
		final long current = getStack(index).getCount();
		final long insertable = StorageUtil.simulateInsert(this.storage, ItemVariant.of(stack), Integer.MAX_VALUE, null);
	...
	public boolean isValid(int index, ItemStack stack) {
		...
		final Storage<ItemVariant> target = this.slotted != null ? slot(index) : this.storage;
	...
	public int insert(int index, ItemStack stack, boolean simulate) {
		...
		try (Transaction transaction = Transaction.openOuter()) {
			final int inserted = (int) target.insert(variant, stack.getCount(), transaction);
	...
	public ItemStack extract(int index, int amount, boolean simulate) {
		...
		try (Transaction transaction = Transaction.openOuter()) {
			final int extracted = (int) target.extract(resource, amount, transaction);
