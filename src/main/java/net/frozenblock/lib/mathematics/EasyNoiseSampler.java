...
public final class EasyNoiseSampler {
    ...
    public static RandomSource threadSafeRandom =
            new ThreadSafeLegacyRandomSource(seed);
    ...
    public static ImprovedNoise perlinBlocking =
            new ImprovedNoise(threadSafeRandom);
    ...
    public static double sample(ImprovedNoise sampler, BlockPos pos,
                                double multiplier, boolean multiplyY,
                                boolean useY) {
        if (useY) {
            if (multiplyY) {
                return sampler.noise(pos.getX() * multiplier,
                        pos.getY() * multiplier, pos.getZ() * multiplier);
            }
            return sampler.noise(pos.getX() * multiplier, pos.getY(),
                    pos.getZ() * multiplier);
        }
        return sampler.noise(pos.getX() * multiplier, 64,
                pos.getZ() * multiplier);
    }

    public static double samplePositive(ImprovedNoise sampler, BlockPos pos,
                                        double multiplier, boolean multiplyY,
                                        boolean useY) {
        double ret = 0;
        if (useY) {
            if (multiplyY) {
                ret = sampler.noise(pos.getX() * multiplier,
                        pos.getY() * multiplier, pos.getZ() * multiplier);
            } else {
                ret = sampler.noise(pos.getX() * multiplier, pos.getY(),
                        pos.getZ() * multiplier);
            }
        } else {
            ret = sampler.noise(pos.getX() * multiplier, 64,
                    pos.getZ() * multiplier);
        }
        if (ret < 0) {
            return ret * -1;
