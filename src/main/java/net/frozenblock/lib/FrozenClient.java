...
public final class FrozenClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        if (FabricLoader.getInstance().isDevelopmentEnvironment()) {
            FlyBySoundHub.autoEntitiesAndSounds.put(EntityType.ARROW,
                    new FlyBySoundHub.FlyBySound(1.0F, 1.0F,
                            SoundSource.NEUTRAL, SoundEvents.AXE_SCRAPE));
        }
        ...
        FabricLoader.getInstance().getEntrypointContainers("frozenlib:client",
                FrozenClientEntrypoint.class).forEach(entrypoint -> {
            try {
    ...
    private static void receiveMovingRestrictionSoundPacket() {
        ClientPlayNetworking.registerGlobalReceiver(
                FrozenMain.MOVING_RESTRICTION_SOUND_PACKET,
                (ctx, handler, byteBuf, responseSender) -> {
                    ...
                    ctx.execute(() -> {
                        ...
                        if (level != null) {
                            ...
                            if (entity != null) {
                                FrozenSoundPredicates.LoopPredicate<?>
                                        predicate =
                                        FrozenSoundPredicates.getPredicate(
                                                predicateId);
                                Minecraft.getInstance().getSoundManager()
                                        .play(new MovingSoundWithRestriction(
                                                entity, sound, category, volume,
                                                pitch, predicate));
                            }
                        }
                    });
                            ...
                            if (entity != null) {
                                FrozenSoundPredicates.LoopPredicate<?>
                                        ...
                                        FrozenSoundPredicates.getPredicate(
                                                predicateId);
                                Minecraft.getInstance().getSoundManager()
                                        .play(new MovingSoundLoopWithRestriction(
                                                entity, sound, category, volume,
                                                pitch, predicate));
                            }
                        }
        ...
        ClientPlayNetworking.registerGlobalReceiver(
                ...
                (ctx, handler, byteBuf, responseSender) -> {
                    ...
                    SoundEvent startingSound =
                            byteBuf.readById(FrozenRegistry.STARTING_SOUND);
                    SoundEvent loopingSound =
                            byteBuf.readById(Registry.SOUND_EVENT);
                    SoundSource category = byteBuf.readEnum(SoundSource.class);
                    float volume = byteBuf.readFloat();
                    float pitch = byteBuf.readFloat();
                    ResourceLocation predicateId =
                            ...
                            if (entity != null) {
                                FrozenSoundPredicates.LoopPredicate<?>
                                        ...
                                        FrozenSoundPredicates.getPredicate(
                                                predicateId);
                                Minecraft.getInstance().getSoundManager()
                                        .play(new StartingSoundInstance(entity,
                                                startingSound, loopingSound,
                                                category, volume, pitch,
                                                predicate,
                                                new MovingSoundLoopWithRestriction(
                                                        entity, loopingSound,
                                                        category, volume, pitch,
                                                        predicate)));
                            }
                            ...
                            if (entity != null) {
                                FrozenSoundPredicates.LoopPredicate<?>
                                        ...
                                        FrozenSoundPredicates.getPredicate(
                                                predicateId);
                                Minecraft.getInstance().getSoundManager()
                                        .play(new MovingFadingDistanceSwitchingSoundLoop(
                                                entity, sound, category, volume,
                                                pitch, predicate, fadeDist,
                                                maxDist, volume, false));
                                Minecraft.getInstance().getSoundManager()
                                        .play(new MovingFadingDistanceSwitchingSoundLoop(
                                                entity, sound2, category,
                                                volume, pitch, predicate,
                                                fadeDist, maxDist, volume,
                                                true));
                            }
                        }
                    });
    ...
    private static void receiveMovingFadingDistanceSoundPacket() {
        ClientPlayNetworking.registerGlobalReceiver(
                FrozenMain.MOVING_FADING_DISTANCE_SOUND_PACKET,
                (ctx, handler, byteBuf, responseSender) -> {
                    int id = byteBuf.readVarInt();
                    SoundEvent sound = byteBuf.readById(Registry.SOUND_EVENT);
                    SoundEvent sound2 = byteBuf.readById(Registry.SOUND_EVENT);
    ...
    private static void receiveFadingDistanceSoundPacket() {
        ClientPlayNetworking.registerGlobalReceiver(
                FrozenMain.FADING_DISTANCE_SOUND_PACKET,
                (ctx, handler, byteBuf, responseSender) -> {
                    ...
                    ctx.execute(() -> {
                        ...
                        if (level != null) {
                            Minecraft.getInstance().getSoundManager()
                                    .play(new FadingDistanceSwitchingSound(
                                            sound, category, volume, pitch,
                                            fadeDist, maxDist, volume, false, x,
                                            y, z));
                            Minecraft.getInstance().getSoundManager()
                                    .play(new FadingDistanceSwitchingSound(
                                            sound2, category, volume, pitch,
                                            fadeDist, maxDist, volume, true, x,
                                            y, z));
                        }
                    });
                });
                ...
                (ctx, handler, byteBuf, responseSender) -> {
                    ...
                    ctx.execute(() -> {
                        ...
                        if (level != null) {
                            Entity entity = level.getEntity(id);
                            if (entity != null) {
                                FlyBySoundHub.FlyBySound flyBySound =
                                        new FlyBySoundHub.FlyBySound(pitch,
                                                volume, category, sound);
    ...
    private static void receiveCooldownChangePacket() {
        ClientPlayNetworking.registerGlobalReceiver(
                FrozenMain.COOLDOWN_CHANGE_PACKET,
                (ctx, handler, byteBuf, responseSender) -> {
                    ...
                    ctx.execute(() -> {
                        ...
                        if (level != null &&
                            ...
                            ((CooldownInterface) Minecraft.getInstance().player.getCooldowns()).changeCooldown(
                                    item, additional);
                        }
                    });
                });
