	...
	public static void init() {
		ServerWorldEvents.LOAD.register((server, level) -> {
			level.registryAccess().lookupOrThrow(Registries.STRUCTURE_SET).listElements().forEach(structureSetReference -> {
				if (structureSetReference.isBound() && (Object) (structureSetReference.value()) instanceof StructureAddExclusionInterface addExclusionInterface) {
					addExclusionInterface.frozenLib$addExclusions(
