...
public class AppendSherds implements RuleBlockEntityModifier {
	...
	public CompoundTag apply(@NotNull RandomSource random, @Nullable CompoundTag nbt) {
		...
		List<Item> orderedDecorations = DecoratedPotBlockEntity.Decorations.load(nbt).sorted().toList();
		...
		DecoratedPotBlockEntity.Decorations processedDecorations = new DecoratedPotBlockEntity.Decorations(
			chosenSherds[0],
			chosenSherds[1],
			chosenSherds[2],
			chosenSherds[3]
