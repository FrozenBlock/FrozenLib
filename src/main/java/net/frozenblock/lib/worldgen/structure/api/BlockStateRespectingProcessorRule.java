...
public class BlockStateRespectingProcessorRule {
	...
	public static final Codec<BlockStateRespectingProcessorRule> CODEC = RecordCodecBuilder.create(
		instance -> instance.group(
				...
				PosRuleTest.CODEC.optionalFieldOf("position_predicate", PosAlwaysTrueTest.INSTANCE).forGetter(rule -> rule.posPredicate),
				BuiltInRegistries.BLOCK.byNameCodec().fieldOf("output_block").forGetter(rule -> rule.outputBlock),
				RuleBlockEntityModifier.CODEC.optionalFieldOf("block_entity_modifier", DEFAULT_BLOCK_ENTITY_MODIFIER).forGetter(rule -> rule.blockEntityModifier)
			)
