...
public class FadingDistanceSwitchingSound
    ...
    public FadingDistanceSwitchingSound(SoundEvent sound, SoundSource category,
                                        float volume, float pitch,
                                        double fadeDist, double maxDist,
                                        float maxVol, boolean isFarSound,
    ...
    public void tick() {
        ...
        if (client.player != null) {
            float distance = (float) Math.sqrt(
                    client.player.distanceToSqr(this.x, this.y, this.z));
            if (distance < this.fadeDist) {
                ...
                float fadeProgress = (float) ((this.maxDist - distance) /
                        (this.maxDist - this.fadeDist));
                this.volume =
                        this.isFarSound ? (1F - fadeProgress) * this.maxVol :
