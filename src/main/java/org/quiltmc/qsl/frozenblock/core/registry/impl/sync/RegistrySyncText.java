...
public class RegistrySyncText {
	...
	private static <T> MutableComponent entryList(List<T> namespacesList, Function<T, Component> toText) {
		final var namespaceText = Component.empty();

		var textLength = 0;
		var lines = 0;
		while (lines < 2 && !namespacesList.isEmpty()) {
			final var max = lines == 0 ? 38 : 30;
			while (textLength < max && !namespacesList.isEmpty()) {
				final var t = toText.apply(namespacesList.remove(0));
				namespaceText.append(t);

				textLength += t.getString().length();

				if (namespacesList.isEmpty()) continue;

				final var alt = (lines + toText.apply(namespacesList.get(0)).getString().length() < max && lines == 1);
			...
			if (!namespacesList.isEmpty() && lines != 1) namespaceText.append("\n");

			textLength = 0;
			lines++;
		}

		if (!namespacesList.isEmpty()) namespaceText.append(
			Component.translatableWithFallback("frozenlib.core.registry_sync.more", "%s more...", namespacesList.size())
		);
	...
	public static Component unsupportedModVersion(List<ModProtocolDef> unsupported, ModProtocolDef missingPrioritized) {
		if (missingPrioritized != null && !missingPrioritized.versions().isEmpty()) {
			final var x = Component.translatableWithFallback(
				"frozenlib.core.registry_sync.require_main_mod_protocol",
			...
			if (ModProtocol.enabled && ModProtocol.prioritizedEntry != null) {
				x.append("\n").append(
					Component.translatableWithFallback(
						"frozenlib.core.registry_sync.main_mod_protocol",
						"You are on %s with protocol version of %s.",
						Component.literal(ModProtocol.prioritizedEntry.displayName()).withStyle(ChatFormatting.GOLD),
						ModProtocol.prioritizedEntry.versions().getInt(0)
					)
				);
			}
			...
			final var namespaceText = entryList(namespacesList, RegistrySyncText::protocolDefEntryText).withStyle(ChatFormatting.GRAY);

			return Component.translatableWithFallback(
				"frozenlib.core.registry_sync.unsupported_mod_protocol",
	...
	private static Component protocolDefEntryText(ModProtocolDef def) {
		...
		final var versions = def.versions();
		if (versions.isEmpty()) {
			version = Component.literal("WHAT??? HOW???");
		} else if (versions.size() == 1) {
			version = Component.literal("" + versions.getInt(0));
		} else {
			version = (MutableComponent) ComponentUtils.formatList(versions.subList(0, Math.min(versions.size(), 4)), n -> Component.literal(n.toString()));
			if (versions.size() > 4) version = version.append(ComponentUtils.DEFAULT_SEPARATOR).append("...");
		}
