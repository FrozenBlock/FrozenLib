...
public class ModProtocol {
	...
	public static void loadVersions() {
		...
		for (var container : FabricLoader.getInstance().getAllMods()) {
			final var data = container.getMetadata();
			final var frozenRegistry = data.getCustomValue("frozenlib_registry");
			...
			if (value.getType() == CustomValue.CvType.OBJECT) {
				...
				if (version != null) add(new ModProtocolDef("mod:" + data.getId(), data.getName(), version, optional));
			} else {
				final var version = decodeVersion("", container, value);
				if (version != null) add(new ModProtocolDef("mod:" + data.getId(), data.getName(), version, false));
			}
		}
	}
	...
	private static IntList decodeVersion(String path, ModContainer container, CustomValue value) {
		...
		if (value.getType() == CustomValue.CvType.NUMBER) {
			final int version = value.getAsNumber().intValue();
			if (version < 0) {
				negativeEntry(path, container, version);
				return null;
			}
